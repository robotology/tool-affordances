...
void ResampleCurve2D(const vector<Point>& curve, vector<Point>& resampled, int N, bool isOpen) {
	assert(curve.size()>0);
	
	resampled.resize(N);
	resampled[0].x = curve[0].x; resampled[0].y = curve[0].y;
	//vector<Point2i> pl; PolyLineMerge(pl,curvex,curvey);

	double curve_length = arcLength(curve, false);
	double resample_size = curve_length / (double)N;
	int curr = 0;
	double dist = 0.0;
	for (int i=1; i<N; ) {		
		assert(curr < curve.size() - 1);
		double last_dist = norm(curve[curr] - curve[curr+1]);
		dist += last_dist;
//		cout << curr << " and " << curr+1 << "\t\t" << last_dist << " ("<<dist<<")"<<endl;
		if (dist >= resample_size) {
			//put a point on line
			double _d = last_dist - (dist-resample_size);
			Point cp(curve[curr].x,curve[curr].y),cp1(curve[curr+1].x,curve[curr+1].y);
			Point dirv = cp1-cp; dirv = dirv * (1.0 / norm(dirv));
//			cout << "point " << i << " between " << curr << " and " << curr+1 << " remaining " << dist << endl;
			assert(i < resampled.size());
...
//				cout << "point " << i << " between " << curr << " and " << curr+1 << " remaining " << dist << endl;
				assert(i < resampled.size());
				resampled[i] = resampled[i-1] + dirv * resample_size;
				dist -= resample_size;
				i++;
			}
		}		
...
{
	int M = int((10.0*sigma+1.0) / 2.0) * 2 - 1;
...
//#pragma omp parallel for
	for (int i=0; i<490; i++) {
		if (!done) {
			...
			printf("Computing CSS for sigma %.2f \n", sigma);
			...
			drawContours(contourimg, contours, 0, Scalar(255,0,0), CV_FILLED);
			...
			if (crossings.size() > 0) {
				for (int c=0; c<crossings.size(); c++) {
					...
					if (c < crossings.size()-1) {
						...
						if (std::abs(crossings[c] - crossings[c+1]) < 5.0) {
							//this is a maxima
