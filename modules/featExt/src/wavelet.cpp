...
//#define SHRINKAGE_TYPE 0
//#define SHRINKAGE_T 50
//--------------------------------
// signum
//--------------------------------
float sgn(float x)
...
void Haar1DNorm(vector<double>& src, vector<double>& dst)
{
    int width = src.size();
	double norm = 1.0f/cv::sqrt(double(width));
    ...
    while(width > 1) {
        ...
        for(int i=0; i < width; i++) {
            dst[i] = (src[2*i] + src[2*i+1]) * cv::sqrt(2.0);
            dst[width + i] = (src[2*i] - src[2*i+1]) * cv::sqrt(2.0);
        }
    }
}

void Haar1D(vector<double>& src, vector<double>& dst)
{
    int width = src.size();
...
//
//--------------------------------
int processWavelet(VideoCapture& capture)
{
    int n = 0;
    const int NIter=4;
    char filename[200];
