...
if plot_figs
    aff_data_fig = 10;
    aff_SOM_fig = 110;                   %if > 100, affVectors will be plotted on top
    ...
    tool_SOM_fig_nontest = 150;          %if > 100, toolpose images will be plotted on top
else    
    aff_data_fig =  10;
...
if saveF
    save_filename = ['som2som_deep_predreal_aff_', robot '_' datadiv_mode num2str(numRuns) '_' feat_t '_' regr_mode];
    if (baseline_test == 1)
...
if strcmp(datadiv_mode,'1out')
    if (aff_data_fig~= 0)
        figure(aff_data_fig)
        plotPredAffVectors(aff_test_pred_affvector,tool_labels,simF)
    end
end