...
if plot_figs
    ...
    tool_SOM_fig_nontest = 50;          %if > 100, toolpose images will be plotted on top
else    
    aff_data_fig =  10;
...
for runI = 1:numRuns
    ...
    if plot_figs
        ...
        toolim_nontest_ids = map_aff2tool(tp_labels_nontest{runI}, toolIms(3,:),1);
        toolIms_nontest = toolIms(:,toolim_nontest_ids);
    else
         toolIms_nontest = [];
    end
    ...
    %[ tool_bmu_train, clust_tool_train_ids, som_coords_tool_train, sM_tool_train]= dimred2map(tool_data_train, tool_labels_train, 'tool', 'som', toolIms, tool_SOM_fig_train);
    [tool_bmu_nontest, clust_tool_nontest_ids, som_coords_tool_nontest, sM_tool_nontest]  = dimred2map(tool_data_nontest{runI}, tp_labels_nontest{runI}, 'tool', 'som', toolIms_nontest, tool_SOM_fig_nontest);
...
if strcmp(datadiv_mode,'1out')
    if (aff_data_fig~= 0)
        figure(aff_data_fig)
        plotPredAffVectors(aff_test_pred_affvector,tp_labels,simF)
    end
end