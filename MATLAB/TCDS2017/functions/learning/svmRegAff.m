...
searchCnt = 1;
while ((abs(deltamse) > epsilon) && (searchCnt < 5))
    bestmse_prev = bestmse;
    prevStepSize = stepSize;
    stepSize = prevStepSize/2;
    %log2c_list(searchCnt,:) = bestLog2c-prevStepSize:stepSize:bestLog2c+prevStepSize;
    log2c_list = bestLog2c-prevStepSize:stepSize:bestLog2c+prevStepSize;
    log2c_list = log2c_list(log2c_list<8);      % c > 100 severly slows down the search
    %log2c_list(searchCnt,:) = [-9 -5 -1 1 5 9];
    %log2g_list(searchCnt,:) = bestLog2g-prevStepSize:stepSize:bestLog2g+prevStepSize;log2g_list(searchCnt,:) = bestLog2g-prevStepSize:stepSize:bestLog2g+prevStepSize;
    ...
    for i = 1:numLog2c(searchCnt)
        log2c = log2c_list(i);%(searchCnt,i);
        for j = 1:numLog2g(searchCnt)
            log2g = log2g_list(j);%(searchCnt,j);
    ...
    fprintf('=====================================================================\n');
    fprintf('\n Best Cross-validation MSE on grid search = %g\n',mse);
...
% #######################
disp(['Training SVR with best parameters']);

bestParam = ['-q  -s 4  -c ', num2str(bestc), ' -t ', num2str(kernel), ' -g ', num2str(bestg)];
